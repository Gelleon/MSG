...
export class TasksService {
  ...
  async handleCron() {
    ...
    try {
      const inactiveRooms =
        await this.roomsService.findInactivePrivateRooms(threshold);

      if (inactiveRooms.length > 0) {
        this.logger.log(
          `Found ${inactiveRooms.length} inactive private rooms. Closing...`,
        );
        ...
        for (const room of inactiveRooms) {
          try {
            ...
            this.chatGateway.server
              .to(room.id)
              .emit('privateSessionClosed', { roomId: room.id });
            ...
            this.logger.log(
              `Closed inactive private session: ${room.id} (${room.name})`,
            );
          } catch (err) {
